...
namespace Kerosene.ORM.Maps.Concrete
{
	...
	internal interface IUberMember : IMapMember
	{
		...
		/// <summary>
		/// If not null the lazy property created for this member.
		/// </summary>
	...
	public class MapMember<T> : IMapMember<T>, IUberMember where T : class
	{
		...
		internal LazyProperty LazyProperty
		{
			get
			{
				if (!_LazyPropertyCaptured && _Map != null)
				{
					if (_Map.ProxyHolder != null)
					{
						_LazyProperty = _Map.ProxyHolder.LazyProperties.Find(x => x.Name == Name);
					}
